...
foreach ($_server in $servers)
{
    ...
    {   # Execute the stored procedure and get the t-log space used
        ...
        {
            [int32]$active_users_session_count = $Row.Item('active_users_session_count');
            if ($user_interactive -eq $true) {Write-Host -ForegroundColor Yellow "active_users_session_count:" $active_users_session_count};   
        }
