...
[string]$message;
[string]$server;
[string]$query = 'EXEC DBA.dbo.MonitorDistributionAgentsCommands;';
...
foreach ($server in $servers)
{
    ...
    {   # Execute the stored procedure 
        ...
        {            
            ...
            # Graylog        
            if($send_graylog -eq $true) {Send-PSGelfUDP -GelfServer $graylog_server -Port $graylog_port -ShortMessage $collector_name -Facility $server -AdditionalField @{distribution_cmds=$cmds;} }            
        }  
    }
    catch [Exception] 
    {
