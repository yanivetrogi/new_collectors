...
[string]$server;
[string]$query = 'EXEC DBA.dbo.MonitorReplicationLogReaderQueue;';
...
foreach ($server in $servers)
{
    ...
    {   # Execute the stored procedure 
        ...
        {            
            [int]$replicated_transactions = $Row.Item('replicated transactions');

            # Graylog        
            if($send_graylog -eq $true) {Send-PSGelfUDP -GelfServer $graylog_server -Port $graylog_port -ShortMessage $collector_name -Facility $server -AdditionalField @{replicated_transactions=$replicated_transactions;} }
        }  
    }
    catch [Exception] 
    {
